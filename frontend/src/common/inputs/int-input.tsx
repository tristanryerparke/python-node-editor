...
function getCommittedIntValue(rawValue: string): number | undefined {
  if (!COMMITTED_INT_PATTERN.test(rawValue)) {
    return undefined;
  }
...
const IntInput = memo(function ControlledIntInput({
  ...
  const handleValueChange = useCallback(
    (values: NumberFormatValues, sourceInfo: SourceInfo) => {
      ...
      if (sourceInfo.source === "event") {
        void onChange(getCommittedIntValue(nextDraftValue));
      }
    },
    [onChange],
  );
  ...
  const handleBlur = () => {
    ...
    setDraftValue((currentDraftValue) => {
      const committedValue = getCommittedIntValue(currentDraftValue);
